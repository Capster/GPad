...
function GPad.Table.tomarkup(t, colors_override)
    return toMarkup(t, colors_override or GPad.Table.markup_colors)
end

function GPad.Table.print(tbl, colors_override)
    local markup = GPad.Table.tomarkup(tbl, colors_override)
    local jn = true
    for oi = 1, #markup do
        local object = markup[oi]
	...
	MsgC(GPad.Table.markup_colors.default,"\n")
end

GPad.PrintTable = function(...) -- so we don't pass extra tables..
	for k,tbl in pairs{...} do
		GPad.Table.print(tbl)
	end
end


function GPad.Table.tohtml(tbl, colors_override)
    return MarkupToHTML(GPad.Table.tomarkup(tbl, colors_override))
end